...
원본은 멘토링 팀 작업이므로, 코드와 Git 이력으로 확인되는 공개 정리 범위만 기록합니다.
...
- 원본 금융 데이터·credential·팀 내부 작업물·대용량 로컬 자료 제외

팀원별 역할은 공개 근거가 없어 추가하지 않았습니다.
...
> **구현 흐름**: 시장·뉴스 데이터 수집 → DB 저장 → 기술 지표·PCA·시계열 데이터셋 → LSTM·GRU 실험. 뉴스 키워드 분석 결과와 모델 결과는 PyQt 검토 GUI에서 함께 확인합니다.
